...
public class StoredProcedureBean extends StoredProcedure {
	...
	Logger logger = LoggerFactory.getLogger(StoredProcedureBean.class);
    ...
    public Map<String, Object> process(final String body, final Exchange exchange) {
        ...
        for (String key : inParameterNames) {
       		finalParameters.put(key,inParameters.get(key));
        }
        
        //inParameters.keySet().retainAll(inParameterNames);
        //System.out.println("Final............inParameterNames=====>" + inParameters 
        //		+ "..getDeclaredParameters..........." + getDeclaredParameters() + "=======>" + finalParameters);
        // ... and then we can finally execute the procedure and return its results.
   		return this.execute(finalParameters);
   	}
}